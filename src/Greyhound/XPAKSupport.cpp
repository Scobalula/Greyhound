...
bool XPAKSupport::ParseXPAK(const std::string& FilePath)
{
    ...
    if (Header.Magic == 0x4950414b)
    {
        ...
        {
            XPAKNames.LoadIndex(IO::Path::Combine(System::Environment::GetApplicationPath(), "package_index\\bo4_ximage.wni").ToCString());
        }
        ...
        for (uint64_t i = 0; i < Header.IndexCount; i++)
        {
            ...
            {
                // Results
                auto ResultBuffer = string(std::string((char*)PropertiesBuffer, (char*)PropertiesBuffer + ReadSize)).Split('\n');

                // Loop and take the ones we need
                for (auto& KeyValue : ResultBuffer)
                {
                    // Check each value
                    if (_strnicmp(KeyValue.ToCString(), "width:", 6) == 0)
                    {
                        // Notice: Width is actually height
                        Height = KeyValue.Substring(7).ToLong();
                    }
                    else if (_strnicmp(KeyValue.ToCString(), "height:", 7) == 0)
                    {
                        // Notice: Height is actually width
                        Width = KeyValue.Substring(8).ToLong();
                    }
                    else if (_strnicmp(KeyValue.ToCString(), "size0:", 6) == 0)
                    {
                        // Notice: Height is actually width
                        Size = KeyValue.Substring(7).ToLong();
                    }
                    else if (_strnicmp(KeyValue.ToCString(), "name:", 5) == 0)
                    {
                        Name = KeyValue.Substring(6);
                        ...
                        if (Header.Version == 11)
                        {
                            ...
                            {
                                // Set new name
                                Name = string::Format("ximage_%llx", Result);
                    ...
                    else if (_strnicmp(KeyValue.ToCString(), "format:", 7) == 0)
                    {
                        Format = KeyValue.Substring(8);
                    }
                    else if (_strnicmp(KeyValue.ToCString(), "type:", 5) == 0)
                    {
                        Type = KeyValue.Substring(6);
                    }
                }
