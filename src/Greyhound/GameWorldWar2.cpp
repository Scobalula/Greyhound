...
bool GameWorldWar2::LoadAssets()
{
    ...
    if (NeedsModels)
    {
        ...
        {
            // Validate and load if need be
            auto ModelName = IO::Path::GetFileName(CoDAssets::GameInstance->ReadNullTerminatedString(Asset.NamePtr).c_str());
    ...
    if (NeedsImages)
    {
        ...
        CoDXPoolParser<uint64_t, WWIIGfxImage>((CoDAssets::GameOffsetInfos[2] + 8), CoDAssets::GamePoolSizes[2], [](WWIIGfxImage& Asset, uint64_t& AssetOffset)
        {
            ...
            auto ImageName = IO::Path::GetFileName(CoDAssets::GameInstance->ReadNullTerminatedString(Asset.NamePtr).c_str());
        ...
        CoDXPoolParser<uint64_t, WWIISoundAlias>((CoDAssets::GameOffsetInfos[3] + 8), CoDAssets::GamePoolSizes[3], [&UniqueEntries](WWIISoundAlias& Asset, uint64_t& AssetOffset)
        {
            ...
            for (uint64_t i = 0; i < Asset.AliasEntryCount; i++)
            {
                ...
                if (SoundFileBase.SoundFileType == 1)
                {
                    ...
                    // Set
                    LoadedSound->AssetName = IO::Path::GetFileName(SoundName.c_str());
    ...
    if (NeedsMaterials)
    {
        ...
        {
            // Validate and load if need be
            auto MaterialName = IO::Path::GetFileName(CoDAssets::GameInstance->ReadNullTerminatedString(Asset.NamePtr).c_str());
            ...
            // Set
            Material->AssetName = MaterialName.Replace("*", "");
...
std::unique_ptr<XImageDDS> GameWorldWar2::ReadXImage(const CoDImage_t* Image)
{
    ...
    // Determine from name
    if (string(Image->AssetName).Contains("_nml"))
    {
        // Set to normal map
...
std::unique_ptr<XSound> GameWorldWar2::ReadXSound(const CoDSound_t* Sound)
{
    ...
    else
    {
        ...
        {
            PackagePath = IO::Path::Combine(PackagePath.c_str(), string::Format("soundfile%d.pak", Sound->PackageIndex));
        }
        else
        {
            ...
            {
                auto DirectoryCheck = IO::Path::Combine(PackagePath.c_str(), Localization.first.c_str());

                // Check for it
                if (IO::Directory::Exists(DirectoryCheck))
                {
                    // Set them
                    PackagePath = DirectoryCheck;
                    PackagePath = IO::Path::Combine(PackagePath.c_str(), string::Format("%ssoundfile%d.pak", Localization.second.c_str(), Sound->PackageIndex));
        ...
        if (!IO::File::Exists(PackagePath.c_str()))
            return nullptr;
...
void GameWorldWar2::LoadXModel(const std::unique_ptr<XModel_t>& Model, const XModelLod_t& ModelLOD, const std::unique_ptr<WraithModel>& ResultModel)
{
    ...
    for (auto& Submesh : ModelLOD.Submeshes)
    {
        ...
        IO::MemoryReader BlendShapesReader;
        ...
        if (StreamMeshInfo.LoadedSurfaceInfoPtr == 0)
        {
            ...
            auto Key = Hashing::XXHash::HashString(StreamMeshInfo.SubmeshKeyHash);
        ...
        for (uint16_t bv = 0; bv < BlendShapeCount; bv++)
        {
            ...
            IO::MemoryReader ShapeVertsReader(CoDAssets::GameInstance->Read(Info.DataPtr1, ToRead, SizeRead), ToRead, false);
            // Verify we read marv.
            if (SizeRead != ToRead)
            {
                continue;
...
std::unique_ptr<XImageDDS> GameWorldWar2::LoadXImage(const XImage_t& Image)
{
    ...
    auto ImageData = PAKSupport::AWExtractImagePackage(IO::Path::Combine(CoDAssets::GamePackageCache->GetPackagesPath().c_str(), ImagePackageName.c_str()).ToCString(), ImageOffset, 0, ResultSize);
