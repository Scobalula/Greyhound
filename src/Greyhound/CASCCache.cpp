...
void CASCCache::LoadPackageCache(const std::string& BasePath)
{
    ...
    try
    {
        ...
        {
            // We only want XPAKs
            if (IO::Path::GetExtension(File.first.c_str()) == ".xpak" && File.second.Exists)
            {
                try
                {
            ...
            for (uint32_t i = 0; i < BlockHeader.Count; i++)
            {
                ...
                if (CompressedFlag == 0x3)
                {
                    ...
                    if (DataBlock != nullptr)
                    {
                        ...
                        auto Result = WraithCompression::DecompressLZ4Block((const int8_t*)DataBlock.get(), (int8_t*)ResultBuffer.get() + TotalDataSize, (uint32_t)BlockSize, 0x2400000);
...
std::unique_ptr<uint8_t[]> CASCCache::ExtractPackageObject(const std::string& PackageName, uint64_t AssetOffset, uint64_t AssetSize, uint32_t& ResultSize)
{
    ...
    while (TotalUncompressedSize != TotalReadUncompressedSize)
    {
        ...
        {
            // Decompress it
            auto DecompressResult = WraithCompression::DecompressLZ4Block((const int8_t*)CompressedData.get(), (int8_t*)ResultBuffer.get() + TotalReadUncompressedSize, CompressedSize, UncompressedSize);
        }
