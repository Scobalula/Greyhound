            ...
            for (uint32_t i = 0; i < Count; i++)
            {
                ...
                if (CompressedFlag == 0x3)
                {
                    ...
                    if (DataBlock != nullptr)
                    {
                        ...
                        auto Result = WraithCompression::DecompressLZ4Block((const int8_t*)DataBlock.get(), DataTemporaryBuffer + TotalDataSize, (uint32_t)BlockSize, 0x2400000);
                ...
                else if (CompressedFlag == 0x0)
                {
                    ...
                    if (DataBlock != nullptr)
                    {
                        ...
                        {
                            DataTemporaryBufferSize += (uint64_t)0x2400000 + BlockSize;
                            DataTemporaryBuffer = (int8_t*)std::realloc(DataTemporaryBuffer, DataTemporaryBufferSize);
                        }
