...
bool GameModernWarfareRM::LoadAssets()
{
    ...
    if (NeedsMaterials)
    {
        ...
        for (uint32_t i = 0; i < MaterialCount; i++)
        {
            ...
            // Validate and load if need be
            auto MaterialName = IO::Path::GetFileName(CoDAssets::GameInstance->ReadNullTerminatedString(MaterialResult.NamePtr).c_str());
...
std::unique_ptr<XImageDDS> GameModernWarfareRM::ReadXImage(const CoDImage_t* Image)
{
    ...
    {
        // Compare name
        if (string(Image->AssetName).EndsWith("_n") || string(Image->AssetName).EndsWith("_nml"))
        {
            // Set
...
std::unique_ptr<XImageDDS> GameModernWarfareRM::LoadXImage(const XImage_t& Image)
{
    ...
    auto ImageData = PAKSupport::AWExtractImagePackage(IO::Path::Combine(CoDAssets::GamePackageCache->GetPackagesPath().c_str(), string(ImagePackageName + ".pak")).ToCString(), ImageStreamInfo.ImageOffset, (ImageStreamInfo.ImageEndOffset - ImageStreamInfo.ImageOffset), ResultSize);
...
std::unique_ptr<XImageDDS> GameModernWarfareRM::LoadXImagePS(const XImage_t& Image)
{
    ...
    if (ImageInfo.Streamed > 0)
    {
        ...
        // Attempt to extract the package asset
        ImageData = PAKSupport::AWExtractImagePackage(IO::Path::Combine(CoDAssets::GamePackageCache->GetPackagesPath().c_str(), ImagePackageName).ToCString(), ImageStreamInfo.Offset, (ImageStreamInfo.Size - ImageStreamInfo.Offset), ResultSize);
    }
    else
    {
