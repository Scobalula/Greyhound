...
bool GameModernWarfare5::LoadAssets()
{
    ...
    if (NeedsModels)
    {
        ...
        ps::PoolParser64(Pool.FirstXAsset, CoDAssets::ParasyteRequest, [](ps::XAsset64& Asset)
        {
            ...
            if (ModelResult.NamePtr > 0)
                ModelName = IO::Path::GetFileName(string(CoDAssets::GameInstance->ReadNullTerminatedString(ModelResult.NamePtr)).Replace("::", "_"));
            else
        ...
        ps::PoolParser64(Pool.FirstXAsset, CoDAssets::ParasyteRequest, [](ps::XAsset64& Asset)
        {
            ...
            if (CoDAssets::GameFlags == SupportedGameFlags::SP)
            {
                ...
                if (MatResult.NamePtr > 0)
                    MaterialName = string(CoDAssets::GameInstance->ReadNullTerminatedString(MatResult.NamePtr)).Replace("*", "");
                else
                ...
                // Set
                LoadedMaterial->AssetName = IO::Path::GetFileName(MaterialName.c_str());
    ...
    if (NeedsSounds)
    {
        ...
        ps::PoolParser64(Pool.FirstXAsset, CoDAssets::ParasyteRequest, [](ps::XAsset64& Asset)
        {
            ...
            LoadedSound->AssetName = IO::Path::GetFileNamePurgeExtensions(SoundName.c_str());
...
const XMaterial_t GameModernWarfare5::ReadXMaterial(uint64_t MaterialPointer)
{
    ...
    if (CoDAssets::GameFlags == SupportedGameFlags::SP)
    {
        ...
        if (MaterialData.NamePtr > 0)
            Result.MaterialName = IO::Path::GetFileName(string(CoDAssets::GameInstance->ReadNullTerminatedString(MaterialData.NamePtr))).Replace("*", "");
        else
...
void GameModernWarfare5::PrepareVertexWeights(IO::MemoryReader& ComplexReader, std::vector<WeightsData>& Weights, const XModelSubmesh_t & Submesh)
{
...
std::unique_ptr<XImageDDS> GameModernWarfare5::LoadXImage(const XImage_t& Image)
{
    ...
    else
    {
        ...
        size_t MipCount = min((size_t)ImageInfo.MipCount, (size_t)32);
        ...
        if (ImageData != nullptr)
        {
            ...
            else if (Result != nullptr && Image.ImageUsage == ImageUsageType::DiffuseMap && ExportManager::Config.GetBool("PatchColor"))
            {
                // Set color patch
...
void GameModernWarfare5::LoadXModel(const std::unique_ptr<XModel_t>& Model, const XModelLod_t& ModelLOD, const std::unique_ptr<WraithModel>& ResultModel)
{
    ...
    if (MeshDataBuffer != nullptr)
    {
        ...
        IO::MemoryReader VertexPosReader;
        IO::MemoryReader VertexNormReader;
        IO::MemoryReader VertexUVReader;
        IO::MemoryReader VertexColorReader;
        IO::MemoryReader VertexWeightReader;
        IO::MemoryReader SimpleWeightReader;
        IO::MemoryReader FaceIndiciesReader;
        ...
        for (auto& Submesh : ModelLOD.Submeshes)
        {
            ...
            for (uint32_t i = 0; i < Submesh.VertexCount; i++)
            {
                ...
                // Read and set UVs
                auto UVU = Math::Half::ToFloat(VertexUVReader.Read<uint16_t>());
                auto UVV = Math::Half::ToFloat(VertexUVReader.Read<uint16_t>());
...
void GameModernWarfare5::LoadXModel(const std::unique_ptr<XModel_t>& Model, const XModelLod_t& ModelLOD, const std::unique_ptr<Assets::Model>& ResultModel)
{
      // Scale to use
    auto ScaleConstant = (1.0f / 0x1FFFFF) * 2.0f;
    // Check if we want Vertex Colors
    ...
    if (MeshDataBuffer != nullptr)
    {
        ...
        uint32_t TotalReadWeights = 0;
        // The maximum weight index
        uint32_t MaximumWeightIndex = ResultModel->Bones.Count() - 1;
        ...
        {
            // Create and grab a new submesh
            auto& Mesh = ResultModel->Meshes.Emplace(Submesh.VertexCount, Submesh.FaceCount);
            ...
            for (uint32_t i = 0; i < Submesh.VertexCount; i++)
            {
                ...
                // Add normal
                auto Normal = QTangent.Unpack(nullptr, nullptr);

                // Apply Color (some models don't store colors, so we need to check ptr below)

                auto Color = Assets::VertexColor(255, 255, 255, 255);

                // Read and set UVs
                auto UVU = Math::Half::ToFloat(VertexUVReader.Read<uint16_t>());
                auto UVV = Math::Half::ToFloat(VertexUVReader.Read<uint16_t>());

                auto UV = Vector2(UVU, UVV);

                auto Vertex = Mesh.Vertices.Emplace(Position, Normal, Color, UV);
                ...
                {
                    // Add new weight
                    Vertex.SetWeight(Assets::VertexWeight(WeightValue.BoneValues[w], WeightValue.WeightValues[w]), weightIndex++);
                }
            }
            ...
            if (ExportColors && Submesh.VertexColorPtr != 0xFFFFFFFF)
            {
                ...
                {
                    // Apply Color (some models don't store colors, so we need to check ptr below)
                    auto Color0 = VertexColorReader.Read<uint8_t>();
                    auto Color1 = VertexColorReader.Read<uint8_t>();
                    auto Color2 = VertexColorReader.Read<uint8_t>();
                    auto Color3 = VertexColorReader.Read<uint8_t>();
                    Mesh.Vertices[i].SetColor(Assets::VertexColor(Color0, Color1, Color2, Color3));
                }
            }
            ...
            for (uint32_t i = 0; i < Submesh.FaceCount; i++)
            {
                ...
                // Add the face
                Mesh.Faces.Add(Assets::Face(Face.Index1, Face.Index2, Face.Index3));
            }
        }
    }
}
...
void GameModernWarfare5::PerformInitialSetup()
{
    ...
    // Load Caches
    CoDAssets::StringCache.LoadIndex(IO::Path::Combine(System::Environment::GetApplicationPath(), "package_index\\fnv1a_xstring.wni").ToCString());
    CoDAssets::StringCache.LoadIndex(IO::Path::Combine(System::Environment::GetApplicationPath(), "package_index\\fnv1a_bones.wni").ToCString());
    CoDAssets::AssetNameCache.LoadIndex(IO::Path::Combine(System::Environment::GetApplicationPath(), "package_index\\fnv1a_xsounds_unverified.wni").ToCString());
    CoDAssets::AssetNameCache.LoadIndex(IO::Path::Combine(System::Environment::GetApplicationPath(), "package_index\\fnv_ximages.wni").ToCString());
    CoDAssets::AssetNameCache.LoadIndex(IO::Path::Combine(System::Environment::GetApplicationPath(), "package_index\\fnv_xmaterials.wni").ToCString());

    // Copy if not exists
    if (!IO::File::Exists(OurPath))
    {
        // Check for Battlenet
        if (IO::File::Exists(IO::Path::Combine(ps::state->GameDirectory.c_str(), "oo2core_8_win64.dll")))
        {
            IO::File::Copy(IO::Path::Combine(ps::state->GameDirectory.c_str(), "oo2core_8_win64.dll"), OurPath);
        }
        else
        {
            IO::File::Copy(IO::Path::Combine(ps::state->GameDirectory.c_str(), "_retail_\\oo2core_8_win64.dll"), OurPath);
        }
    }
}
