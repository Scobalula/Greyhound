...
std::unique_ptr<uint8_t[]> PAKSupport::GhostsExtractImagePackage(const std::string& PackageName, uint64_t AssetOffset, uint64_t AssetSize, uint32_t& ResultSize)
{
    ...
    if (IO::File::Exists(PackageName.c_str()))
    {
        // Open the file
        ...
        if (CompressedData != nullptr)
        {
            ...
            // Decompress
            auto Result = WraithCompression::DecompressZLibBlock((const int8_t*)CompressedData, TempWorkingBuffer, (uint32_t)AssetSize, 30000000);
...
std::unique_ptr<uint8_t[]> PAKSupport::AWExtractImagePackage(const std::string& PackageName, uint64_t AssetOffset, uint64_t AssetSize, uint32_t& ResultSize)
{
    ...
    if (IO::File::Exists(PackageName.c_str()))
    {
        ...
        while (TotalUncompressedSize != TotalReadUncompressedSize)
        {
            ...
            {
                // Decompress it
                auto DecompressResult = WraithCompression::DecompressLZ4Block((const int8_t*)CompressedData, (int8_t*)ResultBuffer.get() + TotalReadUncompressedSize, CompressedSize, UncompressedSize);
