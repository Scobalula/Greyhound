        ...
        ps::PoolParser64(Pool.FirstXAsset, CoDAssets::ParasyteRequest, [](ps::XAsset64& Asset)
        {
            ...
            if (SoundBankInfo.BankFilePointer > 0)
            {
                ...
                for (uint32_t i = 0; i < Header.EntriesCount; i++)
                {
                    ...
                    {
                        // We don't have one
                        EntryName = string::Format("_%llx", Entry.Key);
                    }
                    ...
                    LoadedSound->AssetName = IO::Path::GetFileNamePurgeExtensions(EntryName.c_str());
                    LoadedSound->FullPath = IO::Path::GetDirectoryName(EntryName.c_str());
...
void GameModernWarfare4::TranslateRawfile(const CoDRawFile_t* Rawfile, const std::string& ExportPath)
{
    ...
    IO::StreamWriter AliasWriter(IO::File::Create(IO::Path::Combine(ExportFolder.c_str(), (Rawfile->AssetName + ".csv").c_str())));
    ...
    for (uint64_t j = 0; j < Bank.AliasCount; j++)
    {
        ...
        for (uint64_t k = 0; k < Alias.EntriesCount; k++)
        {
            ...
            AliasWriter.WriteLineFmt("%s,%s,%s", Name, Secondary, FileName);
        }
    }
...
Vector3 TransformNormal(Math::Quaternion quat, Vector3 up)
{
...
Vector3 UnpackNormalQuat(uint32_t packedQuat)
{
    ...
    {
    case 0: return TransformNormal(Math::Quaternion(w, x, y, z), Up);
    case 1: return TransformNormal(Math::Quaternion(x, w, y, z), Up);
    case 2: return TransformNormal(Math::Quaternion(x, y, w, z), Up);
    case 3: return TransformNormal(Math::Quaternion(x, y, z, w), Up);
...
void GameModernWarfare4::PerformInitialSetup()
{
    ...
    if (!IO::File::Exists(OurPath))
        IO::File::Copy(IO::Path::Combine(ps::state->GameDirectory.c_str(), "oo2core_7_win64.dll"), OurPath);
}
