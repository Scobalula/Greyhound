...
void WraithNameIndex::SaveIndex(const std::string& FilePath)
{
    ...
    // Compress the data
    auto ResultSize = WraithCompression::CompressLZ4Block(UnpackedData, PackedData, PackageSize, PackageSize + (PackageSize / 2));
...
{
    // Prepare to load
    if (IO::File::Exists(LoadIndex.c_str()))
    {
        // Prepare a reader
        ...
        if (Reader.Read<uint32_t>() == 0x20494E57)
        {
            ...
            if (CompressedBuffer != nullptr)
            {
                ...
                // Decompress it
                auto DecompressResult = WraithCompression::DecompressLZ4Block(CompressedBuffer, Decompressed, Packed, Unpacked);

                // Check
                if (DecompressResult > 0)
                {
                    // Allocate a reader and parse (This will auto clean up)
                    auto MemReader = IO::MemoryReader(Decompressed, DecompressResult);
