...
bool IPAKSupport::ParseIPAK(const std::string& FilePath)
{
    ...
    if (Header.Magic == 0x4950414b)
    {
        ...
        WraithNameIndex IPAKNames(IO::Path::Combine(System::Environment::GetApplicationPath(), "package_index\\bo2_ipak.wni").ToCString());
        ...
        if (IPAKEntriesSegment.Type == 1)
        {
            ...
            for (uint32_t i = 0; i < IPAKEntriesSegment.EntryCount; i++)
            {
                ...
                {
                    // Format from key
                    AssetName = string::Format("_%llx", Entry.Key).ToCString();
                }
