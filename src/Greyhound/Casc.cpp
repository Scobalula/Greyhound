...
{
    auto IndexFiles = IO::Directory::GetFiles(DataPath.c_str(), "*.idx");

    for (auto IndexFile : IndexFiles)
    {

        //System::Console::WriteLine("LoadIndexFiles(): Loading %s...\n", IO::Path::GetFileName(IndexFile).ToCString());
#if _DEBUG
        printf("LoadIndexFiles(): Loading %s...\n", IO::Path::GetFileName(IndexFile).ToCString());
#endif // DEBUG
        BinaryReader IndexReader;

        if (!IndexReader.Open(IndexFile.ToCString(), true))
            continue;
...
{
    auto DataFileNames = IO::Directory::GetFiles(DataPath.c_str(), "data.*");
    auto DataFileCount = DataFileNames.Count();
    DataFiles = std::make_unique<std::shared_ptr<DataFile>[]>(DataFileCount);

    for (size_t i = 0; i < DataFileCount; i++)
    {
        auto FullPath = string::Format("%s/data.%03d", DataPath.c_str(), i);

        if (!IO::File::Exists(FullPath))
            continue;

        DataFiles[i] = std::make_shared<DataFile>(FullPath.ToCString());
    }
}

void Casc::Container::LoadBuildInfo()
{
    if (!IO::File::Exists(BuildInfoPath.c_str()))
    ...
    for (size_t i = 0; i < 2; i++)
    {
        ...
        //System::Console::WriteLine(Line.c_str());
...
void Casc::Container::LoadConfigInfo()
{
    ...
    auto FolderName = IO::Path::Combine(Path.c_str(), IO::Path::Combine("Data\\config", IO::Path::Combine(BuildKey.substr(0, 2).c_str(), BuildKey.substr(2, 2).c_str())));
    auto FileName = IO::Path::Combine(FolderName, BuildKey.c_str());
    ...
    Reader.Open(FileName.ToCString(), true);
...
Casc::Container::Container(const std::string& path)
{
    ...
    Path = path;
    DataPath = IO::Path::Combine(Path.c_str(), "Data\\data");
    BuildInfoPath = IO::Path::Combine(Path.c_str(), ".build.info");
    Closed = false;
    ...
    for (auto& Span : Spans)
    {
        for (auto& Frame : Span.Frames)
        {
            ...
            case 0x4E:
                ...
                break;
            case 0x5A:
                WraithCompression::DecompressZLibBlock((int8_t*)TempBuffer.get() + 1, (int8_t*)Cache.get() + Frame.VirtualStartOffset, Frame.EncodedSize - 1, Frame.ContentSize);
                break;
            default:
                break;
            }
...
size_t Casc::FileReader::Read(uint8_t* buffer, size_t offset, size_t count)
{
    ...
    while (true)
    {
        ...
        case 0x4E:
            ...
            break;
        case 0x5A:
            WraithCompression::DecompressZLibBlock((int8_t*)TempBuffer.get() + 1, (int8_t*)Cache.get(), frame->EncodedSize - 1, frame->ContentSize);
            break;
        default:
            break;
        }
