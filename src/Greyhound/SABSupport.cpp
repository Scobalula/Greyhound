...
bool SABSupport::ParseSAB(const std::string& FilePath)
{
    ...
    if (!HasNames)
    {
        ...
        if (Header.Version == 0xE) { SABNames.LoadIndex(IO::Path::Combine(System::Environment::GetApplicationPath(), "package_index\\bo2_sab.wni").ToCString()); }
        if (Header.Version == 0xF) { SABNames.LoadIndex(IO::Path::Combine(System::Environment::GetApplicationPath(), "package_index\\bo3_sab.wni").ToCString()); }
    }
    ...
    if (HasNames)
    {
        ...
        if (Header.Version == 0x15)
        {
            ...
            {
                SABNames.LoadIndex(IO::Path::Combine(System::Environment::GetApplicationPath(), "package_index\\fnv1a_xsounds.wni").ToCString());
                ...
                for (uint32_t i = 0; i < Header.EntriesCount; i++)
                {
                    ...
                    {
                        SABFileNames.emplace_back(string::Format("xsound_%llx", Hash));
                    }
                    // Calculate jump
    ...
    case SupportedGames::BlackOps4:
        ...
        // Load
        HandleSABv21(Reader, Header, SABFileNames, SABNames, IO::Path::GetFileName(FilePath.c_str()).ToCString()); break;
    case SupportedGames::ModernWarfare5:
        // Set the game, then load
        CoDAssets::GameID = SupportedGames::ModernWarfare5;
        // Load
        HandleSABv21v2(Reader, Header, SABFileNames, SABNames, IO::Path::GetFileName(FilePath.c_str()).ToCString()); break;
    }
...
std::unique_ptr<XSound> SABSupport::DecodeOpusInterleaved(uint8_t* OpusBuffer, size_t OpusBufferSize, size_t OpusDataOffset, uint32_t FrameRate, uint32_t Channels, uint32_t FrameCount)
{
    ...
    IO::MemoryWriter TempWriter((FrameRate + 4096) * 2 * Channels);
...
{
    // Get Settings
    auto SkipBlankAudio = ExportManager::Config.Get<System::SettingType::Boolean>("skipblankaudio");
...
void SABSupport::HandleSABv21(BinaryReader& Reader, const SABFileHeader& Header, const std::vector<std::string>& NameList, const WraithNameIndex& NameIndex, const std::string FileName)
{
    // Get Settings
    auto SkipBlankAudio = ExportManager::Config.Get<System::SettingType::Boolean>("skipblankaudio");
    auto UseIndex = ExportManager::Config.Get<System::SettingType::Boolean>("usesabindexbo4");
    ...
    for (uint32_t i = 0; i < Header.EntriesCount; i++)
    {
        ...
        if (NameList.size() > 0)
        {
            ...
            if (string(EntryName).StartsWith("xsound_") && UseIndex)
            {
                // Set the name, but remove all extensions first
                LoadedSound->AssetName = string::Format("%s.%i", FileName.c_str(), i);
...
void SABSupport::HandleSABEncryption(BinaryReader& Reader, const std::string& FilePath)
{
    ...
    if (IO::Path::GetFileName(FilePath.c_str()).Contains("zm_genesis"))
    {
        // We must swap out for a decrypted reader
        if (!IO::File::Exists((FilePath + ".wraithdec").c_str()))
        {
