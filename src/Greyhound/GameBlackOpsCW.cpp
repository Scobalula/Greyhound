...
bool GameBlackOpsCW::LoadAssets()
{
    ...
    */
    ...
    Filters.LoadIndex(IO::Path::Combine(System::Environment::GetApplicationPath(), "package_index\\bocw_filters.wni").ToCString());
    ...
    if (NeedsModels)
    {
        ...
        CoDXPoolParser<uint64_t, BOCWXModel>((CoDAssets::GameOffsetInfos[1]), CoDAssets::GamePoolSizes[1], [Filters, NeedsExtInfo](BOCWXModel& Asset, uint64_t& AssetOffset)
        {
            ...
            // Set
            LoadedModel->AssetName         = ModelName.ToCString();
    ...
    if (NeedsSounds)
    {
        ...
        CoDXPoolParser<uint64_t, BOCWSoundAsset>((CoDAssets::GameOffsetInfos[4]), CoDAssets::GamePoolSizes[4], [](BOCWSoundAsset& Asset, uint64_t& AssetOffset)
        {
            ...
            // Validate and load if need be
            auto SoundName = string::Format("xsound_%llx", Asset.NamePtr);
            ...
            // Log it
            CoDAssets::LogXAsset("Sound", SoundName.ToCString());

            // Make and add
            auto LoadedSound = new CoDSound_t();
            // Set the name, but remove all extensions first
            LoadedSound->AssetName    = IO::Path::GetFileNamePurgeExtensions(SoundName);
            LoadedSound->FullPath     = IO::Path::GetDirectoryName(SoundName);
...
{
    // Proxy off
    return LoadXImage(XImage_t(ImageUsageType::DiffuseMap, (uint32_t)0, Image->AssetPointer, Image->AssetName));
}
...
std::unique_ptr<XSound> GameBlackOpsCW::ReadXSound(const CoDSound_t * Sound)
{
    ...
    IO::MemoryWriter TempWriter((SoundData.FrameCount + 4096) * 2 * SoundData.ChannelCount);
...
const XMaterial_t GameBlackOpsCW::ReadXMaterial(uint64_t MaterialPointer)
{
    ...
    Result.TechsetName = string::Format("xtechset_%llx", CoDAssets::GameInstance->Read<uint64_t>(MaterialData.TechsetPtr));
    ...
    for (uint32_t m = 0; m < MaterialData.ImageCount; m++)
    {
        ...
        case 0xE9817F0D:
            ...
            break;
        case 0x7176BF2:
            DefaultUsage = ImageUsageType::AmbientOcclusionMap;
            break;
        }
        ...
        if (MeshInfo.BlendShapeIndexes != 0 && MeshInfo.BlendShapeCounts != 0)
        {
            for (size_t i = 0; i < ((size_t)MeshInfo.ShapeCount + (size_t)MeshInfo.FuckKnowsAgain); i++)
            {
                ...
                for (size_t j = 0; j < (size_t)VertexInfo.Count; j++)
                {
                    ...
                    auto X = Math::Half::ToFloat(ValueForVertex.X);
                    auto Y = Math::Half::ToFloat(ValueForVertex.Y);
                    auto Z = Math::Half::ToFloat(ValueForVertex.Z);

                    Vertices[ValueForVertex.VertexIndex].push_back(std::make_pair(
                        (uint32_t)ShapeIndex,
                        Vector3(
                            Math::Half::ToFloat(ValueForVertex.X),
                            Math::Half::ToFloat(ValueForVertex.Y),
                            Math::Half::ToFloat(ValueForVertex.Z)
                    )));
                }
            }
        }
...
//DOESNT WORK - Because some of the vertex data (specifically Y axis) gets read WAY wrong.
//Despite the fact the function above reads in the SAME DATA and the vertex data is also wrong, somehow the function above fixes the vertex data?
void GameBlackOpsCW::LoadXModel(const XModelLod_t& ModelLOD, const std::unique_ptr<Assets::Model>& ResultModel)
{
    // Check if we want Vertex Colors
    bool ExportColors = ExportManager::Config.GetBool("exportvtxcolor");
    // Read the mesh information
    ...
    if (MeshDataBuffer != nullptr)
    {
        ...
        if (MeshInfo.BlendShapeIndexes != 0 && MeshInfo.BlendShapeCounts != 0)
        {
            for (size_t i = 0; i < ((size_t)MeshInfo.ShapeCount + (size_t)MeshInfo.FuckKnowsAgain); i++)
            {
                ...
                for (size_t j = 0; j < (size_t)VertexInfo.Count; j++)
                {
                    ...
                    auto Y = Math::Half::ToFloat(ValueForVertex.Y);
                    auto Z = Math::Half::ToFloat(ValueForVertex.Z);
                    Vertices[ValueForVertex.VertexIndex].push_back(std::make_pair((uint32_t)ShapeIndex,Vector3(Math::Half::ToFloat(ValueForVertex.X),Math::Half::ToFloat(ValueForVertex.Y),Math::Half::ToFloat(ValueForVertex.Z))));
                }
            }
        }
        ...
        {
            // Create and grab a new submesh
            Assets::Mesh& Mesh = ResultModel->Meshes.Emplace(Submesh.VertexCount, Submesh.FaceCount, 255, 132);

            auto tempMesh = WraithSubmesh();

            // Set the material (COD has 1 per submesh)
            Mesh.MaterialIndices.Add(Submesh.MaterialIndex);
            List<Vector3> positions(Submesh.VertexCount, false);
            ...
            for (uint32_t i = 0; i < Submesh.VertexCount; i++)
            {
                // Make a new vertex
                // Read and assign position
                //auto vertex = WraithVertex();
                //vertex.Position = MeshReader.Read<Vector3>();
                //WraithVerticies.push_back(vertex);
                //positions.EmplaceBack(MeshReader.Read<Vector3>());
                auto& vertex = tempMesh.AddVertex();
                vertex.Position = MeshReader.Read<Vector3>();
                BlendShapeDeltas = Vertices[Submesh.VertexPtr + i];
            }
            // Jump to vertex info data, advance to this submeshes info, seek further for extended vertex info
            MeshReader.SetPosition(MeshInfo.UVOffset + (Submesh.VertexPtr * (HasExtendedVertexInfo ? 24 : 16)));
            ...
            {
                // Grab the reference
                auto vertColour = Assets::VertexColor();
                auto vertNormal = Vector3();
                auto UVLayer = Vector2();

                // Grab the reference
                auto& WraithVertex = tempMesh.Verticies[i];

                // Read vertex data
                auto VertexData = MeshReader.Read<GfxStreamVertex>();

                // Add UV layer
                UVLayer = Vector2(Math::Half::ToFloat(VertexData.UVUPosition), Math::Half::ToFloat(VertexData.UVVPosition));
                // Unpack normal
                vertNormal.X = ((float)(((VertexData.VertexNormal >> 00) & ((1 << 10) - 1)) - 512) / 511.0f);
                vertNormal.Y = ((float)(((VertexData.VertexNormal >> 10) & ((1 << 10) - 1)) - 512) / 511.0f);
                vertNormal.Z = ((float)(((VertexData.VertexNormal >> 20) & ((1 << 10) - 1)) - 512) / 511.0f);

                // Add Colors if we want them
                if (ExportColors)
                {
                    vertColour = Assets::VertexColor(VertexData.Color[0], VertexData.Color[1], VertexData.Color[2], VertexData.Color[3]);
                }
                else
                {
                    vertColour = Assets::VertexColor(0xFF, 0xFF, 0xFF, 0xFF);
                }

                auto vert = Mesh.Vertices.Emplace(WraithVertex.Position, vertNormal, vertColour, UVLayer);
            ...
            for (uint32_t i = 0; i < Submesh.VertexCount; i++)
            {
                ...
                if (((uint8_t)Submesh.WeightCounts[0] & 2) > 0)
                {
                    ...
                    // Add weights for each index if it exists
                    // Mask off bit, seems to be blendshape related
                    Vertex.SetWeight(Assets::VertexWeight(VertexWeight.WeightID1 & ~32768, (VertexWeight.WeightVal1 / 255.0f)), weightIndex++);

                    if (VertexWeight.WeightVal2 > 0)
                        Vertex.SetWeight(Assets::VertexWeight((VertexWeight.WeightID2 & ~32768), (VertexWeight.WeightVal2 / 255.0f)), weightIndex++);
                    if (VertexWeight.WeightVal3 > 0)
                        Vertex.SetWeight(Assets::VertexWeight((VertexWeight.WeightID3 & ~32768), (VertexWeight.WeightVal3 / 255.0f)), weightIndex++);
                    if (VertexWeight.WeightVal4 > 0)
                        Vertex.SetWeight(Assets::VertexWeight((VertexWeight.WeightID4 & ~32768), (VertexWeight.WeightVal4 / 255.0f)), weightIndex++);
                }
                else
                {
                    // Simple weight
                    Vertex.SetWeight(Assets::VertexWeight(0, 1.0), weightIndex++);
                }
            }
            ...
            for (uint32_t i = 0; i < Submesh.FaceCount; i++)
            {
                ...
                // Add the face
                Mesh.Faces.EmplaceBack(Face.Index1, Face.Index2, Face.Index3);
            }
        }
    }
}
...
std::string GameBlackOpsCW::LoadStringEntry(uint64_t Index)
{
    ...
    if (StringEntry != StringCache.NameDatabase.end())
        ...
        return string::Format("xstring_%llx", StringHash).ToCString();
}
void GameBlackOpsCW::PerformInitialSetup()
{
    ...
    StringCache.LoadIndex(IO::Path::Combine(System::Environment::GetApplicationPath(),       "package_index\\fnv1a_xstring.wni").ToCString());
    // Prepare to copy the oodle dll
    auto OurPath = IO::Path::Combine(System::Environment::GetApplicationPath(), "oo2core_8_win64.dll");
    // Copy if not exists
    if (!IO::File::Exists(OurPath))
        IO::File::Copy(IO::Path::Combine(IO::Path::GetDirectoryName(CoDAssets::GameInstance->GetProcessPath().c_str()), "oo2core_8_win64.dll"), OurPath);
}