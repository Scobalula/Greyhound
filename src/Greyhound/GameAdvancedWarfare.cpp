...
std::unique_ptr<XImageDDS> GameAdvancedWarfare::LoadXImage(const XImage_t& Image)
{
    ...
    auto ImageData = PAKSupport::AWExtractImagePackage(IO::Path::Combine(CoDAssets::GamePackageCache->GetPackagesPath().c_str(), (ImagePackageName + ".pak").c_str()).ToCString(), ImageStreamInfo.ImageOffset, (ImageStreamInfo.ImageEndOffset - ImageStreamInfo.ImageOffset), ResultSize);
