...
void IPAKCache::LoadPackageCache(const std::string& BasePath)
{
    ...
    auto PathIPAKFiles = IO::Directory::GetFiles(BasePath.c_str(), "*.ipak");

    // Iterate over file paths
    for (auto& IPAKFile : PathIPAKFiles)
    {
        // Load it
        this->LoadPackage(IPAKFile.ToCString());
    }
            ...
            for (uint32_t i = 0; i < BlockHeader.Count; i++)
            {
                ...
                if (CompressedFlag == 0x1)
                {
                    ...
                    if (DataBlock != nullptr)
                    {
                        ...
                        auto Result = WraithCompression::DecompressLZO1XBlock((const int8_t*)DataBlock, DataTemporaryBuffer + TotalDataSize, (uint32_t)BlockSize, 0x1000000);
