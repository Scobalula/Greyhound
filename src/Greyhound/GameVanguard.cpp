...
bool GameVanguard::LoadAssets()
{
    ...
    {
#if _DEBUG
        IO::StreamWriter Writer = IO::StreamWriter(IO::File::OpenWrite("vg_sound_names.csv"));
#endif
...
#endif
        {
            ...
            // Get Settings
            auto SkipBlankAudio = ExportManager::Config.GetBool("skipblankaudio");
            ...
            if (SoundBankInfo.BankFilePointer > 0)
            {
                ...
                for (uint32_t i = 0; i < Header.EntriesCount; i++)
                {
                    ...
                    {
                        LoadedSound->AssetName = IO::Path::GetFileNamePurgeExtensions(NameResult->second.c_str());
                        LoadedSound->FullPath = IO::Path::GetDirectoryName(NameResult->second.c_str());
                    }
                    else
                    {
                        LoadedSound->AssetName = IO::Path::GetFileNamePurgeExtensions(string::Format("xsound_%llx", Entry.Key));
                        LoadedSound->FullPath = IO::Path::GetDirectoryName(LoadedSound->AssetName.c_str());
                    }


                    // Log it
                    CoDAssets::LogXAsset("Sound", IO::Path::Combine(LoadedSound->FullPath.c_str(), LoadedSound->AssetName.c_str()).ToCString());

#if _DEBUG
                    Writer.WriteLineFmt("%llx,%s", Entry.Key, IO::Path::Combine(LoadedSound->FullPath.c_str(), LoadedSound->AssetName.c_str()).ToCString());
#endif 
...
#if _DEBUG
                    Writer.WriteLineFmt("%llx,%s", Entry.Key, IO::Path::Combine(LoadedSound->FullPath.c_str(), LoadedSound->AssetName.c_str()));
#endif 
    ...
    if (NeedsRawFiles)
    {
        ...
        ps::PoolParser64(Pool.FirstXAsset, CoDAssets::ParasyteRequest, [](ps::XAsset64& Asset)
        {
            ...
            // Set
            LoadedRawfile->AssetName = IO::Path::GetFileName(RawfileName.c_str());
            LoadedRawfile->RawFilePath = IO::Path::GetDirectoryName(RawfileName.c_str());
    ...
    if (NeedsMaterials)
    {
        ...
        ps::PoolParser64(Pool.FirstXAsset, CoDAssets::ParasyteRequest, [](ps::XAsset64& Asset)
        {
            ...
            // Set
            LoadedMaterial->AssetName = IO::Path::GetFileName(string(MaterialName).Replace("*", ""));
...
const XMaterial_t GameVanguard::ReadXMaterial(uint64_t MaterialPointer)
{
    ...
    Result.MaterialName = string(Result.MaterialName).Replace("*", "");
...
std::unique_ptr<XImageDDS> GameVanguard::LoadXImage(const XImage_t& Image)
{
    ...
    size_t MipCount = min((size_t)ImageInfo.UnkByte6, (size_t)32);
...
Vector3 VanguardTransformNormal(Math::Quaternion quat, Vector3 up)
{
...
Vector3 VanguardUnpackNormalQuat(uint32_t packedQuat)
{
    ...
    {
    case 0: return VanguardTransformNormal(Math::Quaternion(w, x, y, z), Up);
    case 1: return VanguardTransformNormal(Math::Quaternion(x, w, y, z), Up);
    case 2: return VanguardTransformNormal(Math::Quaternion(x, y, w, z), Up);
    case 3: return VanguardTransformNormal(Math::Quaternion(x, y, z, w), Up);
    ...
    if (MeshDataBuffer != nullptr)
    {
        ...
        for (auto& Submesh : ModelLOD.Submeshes)
        {
            ...
            for (uint32_t i = 0; i < Submesh.VertexCount; i++)
            {
                ...
                // Add normal
                // Vanguard normals seem to be inverted???
            ...
            if (Submesh.BlendShapesPtr != 0)
            {
                ...
                auto BlendShapeReader   = IO::MemoryReader(CoDAssets::GameInstance->Read(MeshBlendShapeInfo.VertexShapes, MeshBlendShapeInfo.VertexShapesDataSize, ResultSize), MeshBlendShapeInfo.VertexShapesDataSize);
                ...
                for (size_t i = 0; i < NumVertsTotal; i++)
                {
                    ...
                    for (size_t j = 0; j < ShapeCount; j++)
                    {
                        ...
                        auto X = Math::Half::ToFloat(BlendShapeReader.Read<uint16_t>());
                        auto Y = Math::Half::ToFloat(BlendShapeReader.Read<uint16_t>());
                        auto Z = Math::Half::ToFloat(BlendShapeReader.Read<uint16_t>());
...
{
    // Load Caches
    StringCache.LoadIndex(IO::Path::Combine(System::Environment::GetApplicationPath(), "package_index\\fnv1a_xstrings.wni").ToCString());
    AssetNameCache.LoadIndex(IO::Path::Combine(System::Environment::GetApplicationPath(), "package_index\\fnv1a_xsounds_unverified.wni").ToCString());
