...
std::unique_ptr<XImageDDS> GameInfiniteWarfare::ReadXImage(const CoDImage_t* Image)
{
    ...
    // Determine from name
    if (string(Image->AssetName).EndsWith("_nog") || string(Image->AssetName).EndsWith("_ng") || string(Image->AssetName).EndsWith("_n") || string(Image->AssetName).EndsWith("_nml"))
    {
        // Set to normal map
        Usage = ImageUsageType::NormalMap;
    }
    else if (string(Image->AssetName).EndsWith("_cs"))
    {
        // Set to diffuse map
...
std::unique_ptr<XImageDDS> GameInfiniteWarfare::LoadXImage(const XImage_t& Image)
{
    ...
    auto ImageData = PAKSupport::IWExtractImagePackage(IO::Path::Combine(CoDAssets::GamePackageCache->GetPackagesPath().c_str(), ImagePackageName.c_str()).ToCString(), ImageStreamInfo.ImageOffset, (ImageStreamInfo.ImageEndOffset - ImageStreamInfo.ImageOffset), ResultSize);
...
std::unique_ptr<XImageDDS> GameInfiniteWarfare::LoadXImagePS(const XImage_t& Image)
{
    ...
    if (ImageInfo.Streamed > 0)
    {
        ...
        // Read image package name
        auto ImagePackageName = string::Format("imagefile%i.pak", ImageStreamInfo.PackageIndex);
        // Attempt to extract the package asset
        ImageData = PAKSupport::IWExtractImagePackage(IO::Path::Combine(CoDAssets::GamePackageCache->GetPackagesPath().c_str(), ImagePackageName).ToCString(), ImageStreamInfo.StartOffset, (ImageStreamInfo.EndOffset - ImageStreamInfo.StartOffset), ResultSize);
    }
    else
    {
        // Use loaded data
