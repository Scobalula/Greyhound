...
public:
    ...
    static void LoadXModel(const std::unique_ptr<XModel_t>& Model, const XModelLod_t& ModelLOD, const std::unique_ptr<Assets::Model>& ResultModel);
...
private:
    ...
    static void PrepareVertexWeights(IO::MemoryReader& ComplexReader, std::vector<WeightsData>& Weights, const XModelSubmesh_t& Submesh);
};