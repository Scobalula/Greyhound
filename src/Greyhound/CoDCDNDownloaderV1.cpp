...
bool CoDCDNDownloaderV1::Initialize(const std::string& gameDirectory)
{
	...
	// Check for Casc
	if (IO::File::Exists((gameDirectory + "\\.build.info").c_str()))
	{
...
std::unique_ptr<uint8_t[]> CoDCDNDownloaderV1::ExtractCDNObject(uint64_t cacheID, size_t sizeOfBuffer, size_t& resultSize)
{
	...
	auto url = string::Format("%s/21/%02x/%016llx_%08llx_%s", CoDV1CDNURL, (uint8_t)entry.Hash, entry.Hash, entry.Size, entry.Flags ? "1" : "0");
	auto result = Client.DownloadData(url.ToCString());
