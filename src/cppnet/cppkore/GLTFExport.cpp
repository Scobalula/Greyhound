...
void GLTF::ExportGLTF(const WraithModel& Model, const std::string& FileName, bool SupportsScale)
{
	...
	for (auto& Bone : Model.Bones)
	{
		...
		Math::Matrix Transform = Math::Matrix::CreateFromQuaternion(Bone.GlobalRotation);
	...
	for (auto& Mesh : Model.Submeshes)
	{
		...
		for (auto& Vertex : Mesh.Verticies)
		{
			...
			// Check Min/Max
			MinPos.X = min(Vertex.Position.X, MinPos.X);
			MinPos.Y = min(Vertex.Position.Y, MinPos.Y);
			MinPos.Z = min(Vertex.Position.Z, MinPos.Z);
			MaxPos.X = max(Vertex.Position.X, MaxPos.X);
			MaxPos.Y = max(Vertex.Position.Y, MaxPos.Y);
			MaxPos.Z = max(Vertex.Position.Z, MaxPos.Z);
		}
		...
		for (size_t c = 0; c < NumWeights; c++)
		{
			...
			GltfPrim.attributes[string::Format("JOINTS_%llu", c).ToCString()] = JointsAccessor;
			...
			GltfPrim.attributes[string::Format("WEIGHTS_%llu", c).ToCString()] = WeightsAccessor;
