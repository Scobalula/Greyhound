...
std::unique_ptr<DirectX::ScratchImage> Image::GenerateDirectXScratch(int8_t* ImageBuffer, uint64_t ImageSize, ImageFormat InFormat)
{
    // Allocate a new image for loading
    auto Image = std::make_unique<DirectX::ScratchImage>();
    // Allocate a buffer for metadata
...
std::unique_ptr<DirectX::ScratchImage> Image::GenerateDirectXScratch(const std::shared_ptr<int8_t[]>& ImageBuffer, uint64_t ImageSize, ImageFormat InFormat)
{
    // Ship off to buffer convert, just get pointer to data
    return GenerateDirectXScratch(ImageBuffer.get(), ImageSize, InFormat);
}
...
bool Image::ConvertImageFile(const std::string& InputFile, ImageFormat InFormat, const std::string& OutputFile, ImageFormat OutFormat, ImagePatch Patch)
{
    ...
    case ImageFormat::Standard_TIFF:
        // Load via WIC
        Result = DirectX::LoadFromWICFile(string(InputFile).ToWString(), DirectX::WIC_FLAGS::WIC_FLAGS_NONE, &ImageMetadata, *Image);
        // Just exit the switch
        break;
    case ImageFormat::Standard_TGA:
        // Load via function
        Result = DirectX::LoadFromTGAFile(string(InputFile).ToWString(), &ImageMetadata, *Image);
        // Just exit the switch
        break;
    case ImageFormat::DDS_WithHeader:
        // Load via function
        Result = DirectX::LoadFromDDSFile(string(InputFile).ToWString(), DirectX::DDS_FLAGS::DDS_FLAGS_NONE, &ImageMetadata, *Image);
        // Just exit the switch
        break;
    default:
        // Failed to load the image, format not supported
...
bool Image::ConvertToFormat(std::unique_ptr<DirectX::ScratchImage>& Image, DirectX::TexMetadata ImageMetadata, const std::string& OutputFile, ImageFormat OutFormat, ImagePatch Patch)
{
    ...
    if (OutputFile != "")
    {
        ...
        {
            // Save it to a file
            DirectX::SaveToDDSFile(FirstImage, ImageCount, ImageMetadata, DirectX::DDS_FLAGS::DDS_FLAGS_NONE, string(OutputFile).ToWString());
        }
        else
        {
            ...
            case ImageFormat::Standard_TGA:
                // Write to a TGA file
                DirectX::SaveToTGAFile(FirstImage[0], string(OutputFile).ToWString());
                // We can exit out
                break;
            case ImageFormat::Standard_BMP:
                // Write to a BMP file
                DirectX::SaveToWICFile(FirstImage, ImageCount, DirectX::WIC_FLAGS::WIC_FLAGS_NONE, DirectX::GetWICCodec(DirectX::WICCodecs::WIC_CODEC_BMP), string(OutputFile).ToWString(), nullptr, nullptr);
                // We can exit out
                break;
            case ImageFormat::Standard_GIF:
                // Write to a GIF file
                DirectX::SaveToWICFile(FirstImage, ImageCount, DirectX::WIC_FLAGS::WIC_FLAGS_NONE, DirectX::GetWICCodec(DirectX::WICCodecs::WIC_CODEC_GIF), string(OutputFile).ToWString(), nullptr, nullptr);
                // We can exit out
                break;
            case ImageFormat::Standard_PNG:
                // Write to a PNG file
                DirectX::SaveToWICFile(FirstImage, ImageCount, DirectX::WIC_FLAGS::WIC_FLAGS_NONE, DirectX::GetWICCodec(DirectX::WICCodecs::WIC_CODEC_PNG), string(OutputFile).ToWString(), nullptr, nullptr);
                // We can exit out
                break;
            case ImageFormat::Standard_JPEG:
                // Write to a JPEG file
                DirectX::SaveToWICFile(FirstImage, ImageCount, DirectX::WIC_FLAGS::WIC_FLAGS_NONE, DirectX::GetWICCodec(DirectX::WICCodecs::WIC_CODEC_JPEG), string(OutputFile).ToWString(), nullptr, [&](IPropertyBag2* props)
                    {
            ...
            case ImageFormat::Standard_TIFF:
                ...
                DirectX::SaveToWICFile(FirstImage, ImageCount, DirectX::WIC_FLAGS::WIC_FLAGS_NONE, DirectX::GetWICCodec(DirectX::WICCodecs::WIC_CODEC_TIFF), string(OutputFile).ToWString(), nullptr, [&](IPropertyBag2* props)
                    {
