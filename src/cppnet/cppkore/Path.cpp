...
namespace IO
{
	...
	string Path::GetDirectoryName(const string& Path)
	{
		...
		if (cLength > rLength)
		{
			...
			return std::string(Path.ToCString()).substr(0, cLength).c_str();
		}
	...
	string Path::GetFileNamePurgeExtensions(const string& Path)
	{
		auto fPath = Path::GetFileName(Path);

		auto fExt = fPath.IndexOf(".");
		if (fExt != string::InvalidPosition)
			return fPath.Substring(0, fExt);
		else
			return fPath;
