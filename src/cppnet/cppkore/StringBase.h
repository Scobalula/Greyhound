...
public:
	...
	static constexpr StringBase<Tchar> DurationToReadableTime(std::chrono::milliseconds Ms);
...
template<class Tchar>
inline constexpr int64_t  StringBase<Tchar>::ToLong() const
{
	// The result
	int64_t Parsed = -1;
	// Convert it
	...
	{
		// Convert the result
		Parsed = std::stol(this->_Buffer, nullptr);
	}
	catch (...)
	{
...
template<class Tchar>
inline constexpr StringBase<Tchar> StringBase<Tchar>::DurationToReadableTime(std::chrono::milliseconds Ms)
{
	...
	StringBase<Tchar> HoursFmt = (Hours.count() > 0) ? StringBase<Tchar>::Format("%dh ", Hours.count()) : StringBase<Tchar>("");
	StringBase<Tchar> MinutesFmt = (Minutes.count() > 0) ? StringBase<Tchar>::Format("%dm ", Minutes.count()) : StringBase<Tchar>("");
	StringBase<Tchar> SecondsFmt = (Seconds.count() > 0) ? StringBase<Tchar>::Format("%ds ", Seconds.count()) : StringBase<Tchar>("");
