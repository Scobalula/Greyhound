...
namespace Assets
{
	...
	enum class TranscodeType
	{
		...
		NormalMapBC5OpenGl,
		//Strips the Alpha (specular) from a color map
		ColorMap
	};
	...
	public:
		...
		bool ConvertToFormat(ImageFormat OutFormat, ImagePatch Patch);
		void PatchNormalFromBumpmap();
		void PatchNormalFromCompressed();
		void PatchNormalCODFromNOG();
		void PatchAlphaChannel();
