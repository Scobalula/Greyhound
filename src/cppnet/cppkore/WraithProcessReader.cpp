...
bool ProcessReader::Attach(const std::string& ProcessName)
{
    ...
    // Convert to wide
    auto WideName = string(ProcessName).ToWString();
    ...
    if (Process32First(Snapshot, &Entry) == TRUE)
    {
        ...
        {
            // Compare the name
            if (_wcsnicmp(Entry.szExeFile, WideName.ToCString(), WideName.Length()) == 0)
            {
                // We found it
        ...
        if (EnumProcessModulesEx(ProcessHandle, hMods, sizeof(hMods), &cbNeeded, IsWin32 ? LIST_MODULES_32BIT : LIST_MODULES_DEFAULT))
        {
            for (uint32_t i = 0; i < (cbNeeded / sizeof(HMODULE)); i++)
            {
                ...
                if (GetModuleFileNameEx(ProcessHandle, hMods[i], szModName, sizeof(szModName) / sizeof(TCHAR)))
                {
                    ...
                    // Convert and return
                    auto FileName = IO::Path::GetFileName(WStringConvert.to_bytes(szModName).c_str()).ToLower().ToCString();
