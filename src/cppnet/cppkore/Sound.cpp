...
bool Sound::ConvertSoundFile(const std::string& InputFile, SoundFormat InFormat, const std::string& OutputFile, SoundFormat OutFormat)
{
    ...
    if (IO::File::Exists(InputFile.c_str()))
    {
        // Allocate a reader
        ...
        {
            // Continue
            auto Result = ConvertSoundMemory((int8_t*)DataBuffer, ResultSize, InFormat, OutputFile, OutFormat);
            // Clean up
            delete[] DataBuffer;
            // Return result
            return Result;
...
bool Sound::TranscodeWAVToFlac(int8_t* SoundBuffer, uint64_t SoundSize, const std::string& OutputFile)
{
    ...
    // Read WAV info
    auto WAVInfoReader = IO::MemoryReader(SoundBuffer, SoundSize, true);
